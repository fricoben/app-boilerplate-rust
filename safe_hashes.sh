...
test_transaction() {
    local chain_id=1
    local safe_address="0x88Ffb774b8583c1C9A2b71b7391861c0Be253993"
    local to="0xde0b295669a9fd93d5f28d9ec85e40f4cb697bae"
    local value=123000000000000000
    local data="0x"
    local operation=0
    local safe_tx_gas=0
    local base_gas=0
    local gas_price=0
    local gas_token="0000000000000000000000000000000000000000"
    local refund_receiver="0000000000000000000000000000000000000000"
    local nonce=1
    local version="1.3.0"

    echo "=== Testing with hardcoded transaction data ==="
    ...
    echo "Safe Tx Gas: $safe_tx_gas"
    echo "Base Gas: $base_gas"
    echo "Gas Price: $gas_price"
    echo "Gas Token: $gas_token"
    echo "Refund Receiver: $refund_receiver"
    echo "Nonce: $nonce"
    echo "Version: $version"
    echo

    # Calculate domain hash
    local domain_separator_typehash="0x47e79534a245952e8b16893a336b85a3d9ea9fa8c573f3d803afb92a79469218"
    local domain_data=$(cast abi-encode "DomainData(bytes32,uint256,address)" \
        "$domain_separator_typehash" \
        "$chain_id" \
        "$safe_address")
    echo "Domain Data: $domain_data"
    local domain_hash=$(cast keccak "$domain_data")
    echo "Domain Hash: $domain_hash"

    # Calculate transaction hash
    echo "Data to hash: $data"
    local data_hash=$(cast keccak "$data")
    echo "Data after Hash: $data_hash"
    local safe_tx_typehash="0xbb8310d486368db6bd6f849402fdd73ad53d316b5a4b2644ad6efe0f941286d8"
    local tx_encoded=$(cast abi-encode "SafeTxStruct(bytes32,address,uint256,bytes32,uint8,uint256,uint256,uint256,address,address,uint256)" \
        "$safe_tx_typehash" \
        "$to" \
        "$value" \
        "$data_hash" \
        ...
        "$refund_receiver" \
        "$nonce")
    local tx_hash=$(cast keccak "$tx_encoded")
    echo "Transaction Hash: $tx_hash"

    # Calculate Safe transaction hash
    local safe_tx_hash_data="0x1901"
    safe_tx_hash_data+=$(echo -n "$domain_hash" | sed 's/0x//')
    safe_tx_hash_data+=$(echo -n "$tx_hash" | sed 's/0x//')
    local safe_tx_hash=$(cast keccak "$safe_tx_hash_data")
...
# Add a new option for test mode
test_mode=false

# Utility function to display the usage information.
usage() {
    cat <<EOF
Usage: $0 [--help] [--list-networks] [--test]
...
Options:
  ...
  --test              Run with test transaction data
...
calculate_hashes() {
    ...
    # Log the SAFE_TX_TYPEHASH
    echo "SAFE_TX_TYPEHASH: $SAFE_TX_TYPEHASH"
